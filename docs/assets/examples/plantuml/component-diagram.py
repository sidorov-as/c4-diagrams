...
with ComponentDiagram(
    ...
    payment_gateway_api = ComponentExt(
        "Payment Gateway API",
    ...
    order_events_bus = ComponentQueue(
        "Order Events Bus",
    ...
    (
        order_controller
        >> Rel("Invokes", technology="Python call", tags=["SyncCall"])
        >> order_app_service
    )
    (
        order_app_service
        >> Rel("Checks stock via", technology="Python call", tags=["SyncCall"])
        >> inventory_checker
    )
    (
        order_app_service
        >> Rel(
            "Requests payment through",
    ...
    (
        order_app_service
        >> Rel("Publishes events to", technology="Kafka", tags=["AsyncFlow"])
        >> order_events_bus
    )
