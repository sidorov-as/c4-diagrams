...
with ContainerDiagram(title="Online Shop - Container Diagram") as diagram:
    ...
    payment_provider = SystemExt(
        ...
        type_="External System",
        tags=["ExternalSystem"],
        alias="payment_provider",
    )
    recommendation_api = ContainerExt(
        "Recommendation API",
    ...
    shipping_events = ContainerQueueExt(
    ...
    (
        web_app
        >> Rel("Calls", technology="HTTPS/JSON", tags=["SyncRequest"])
        >> backend_api
    )
    (
        backend_api
        >> Rel("Reads and writes", technology="SQL", tags=["DataAccess"])
        >> orders_db
    )
    (
        backend_api
        >> Rel(
            "Publishes order events", technology="Kafka", tags=["AsyncRequest"]
        )
        >> order_events
    )
    (
    ...
    (
        backend_api
        >> Rel("Checks fraud signals", technology="JDBC", tags=["ExternalCall"])
        >> fraud_db
    )
    (
        shipping_events
        >> Rel(
            "Delivers shipping updates",
            technology="Kafka",
            tags=["AsyncRequest"],
        )
        >> backend_api
    ...
    (
        support_agent
        >> Rel("Queries order state", technology="HTTPS", tags=["SupportFlow"])
        >> backend_api
    )
